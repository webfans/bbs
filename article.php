...
if (isset($_GET['id'])){
    ...
    if (!!$rows){
      //创建一个数组存放数据
        ...
        $_rows=fetch_array("select u_id,u_sex,u_face,u_email,u_url from bbs.bbs_user WHERE u_username='{$_html['username_subject']}'");
        if (!!$_rows){
           //读取用户信息
...
#START回帖楼层开始START#
     ...
     {      $_html['reply_id']=$rows['art_id'];
            ...
            $_html['date']=$rows['art_date'];
        //拿出用户名去查用户信息(email,url)
        $_rows=fetch_array("select u_id,u_sex,u_face,u_email,u_url from bbs.bbs_user WHERE u_username='{$_html['username_reply']}'");
        if (!!$_rows){
            //读取用户信息
            $_html['userid']=$_rows['u_id'];
            $_html['sex']=$_rows['u_sex'];
            ...
            elseif ($floor==3&&$_html['username_reply']!==$_html['username_subject']){
    ...
    ?>
        ...
        <div class="re">
            ...
            <div class="content">
                <div class="user">
                    <span><?php echo $_html['reply_modify']?>&nbsp;<?php echo $floor+(($_page-1)*$_pagesize);//楼层算法?>楼</span><?php echo $_html['username']?>| 发表于：<?php echo $_html['date']?>
                </div>
